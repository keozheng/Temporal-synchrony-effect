...
for c = 1:length(componentsToPlot)
    ...
    xlabel('Time to stimulus onset (ms)') % Modified by Keo 20200529
...
%     yposs = [0.9 0.65 0.45 0.25];
    yposs = [0.92 0.67 0.44 0.24]; % Modified by Keo 20200529
    ...
    for m = intersect(1:4, unique(options.whichMarg(componentsToPlot))) 
        ...
        annotation('rectangle', [offsetX-0.006 pos(2) 0.015 pos(4)], ...
        ...
        annotation('textarrow', offsetX*[1 1], yposs(row)*[1 1], ...
            ...
            'TextRotation', 90, 'color',[0.9 0.9 0.9]);
    end
end
